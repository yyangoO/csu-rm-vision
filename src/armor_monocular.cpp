...
void ArmorMono::find_armor(Params params)
{
    ...
    for(size_t lb_idx = 0; lb_idx < light_bars.size(); lb_idx++)
    {
        ...
        {
            if((light_bars.at(lb_idx).angle > params.armor_mono_proc_val.armor_retio_simita) \
                && (light_bars.at(lb_idx).angle < 1.5708))
                light_bars.at(lb_idx).angle = 1.5708;
            if((light_bars.at(lb_idx).angle > -1.5708) && (light_bars.at(lb_idx).angle \
                < -params.armor_mono_proc_val.armor_retio_simita))
                light_bars.at(lb_idx).angle = 1.5708;
            if((light_bars.at(lb_idx_idx).angle > params.armor_mono_proc_val.armor_retio_simita) \
                && (light_bars.at(lb_idx_idx).angle < 1.5708))
                light_bars.at(lb_idx_idx).angle = 1.5708;
            if((light_bars.at(lb_idx_idx).angle > -1.5708) && (light_bars.at(lb_idx_idx).angle \
                < params.armor_mono_proc_val.armor_retio_simita))
                light_bars.at(lb_idx_idx).angle = 1.5708;
               ...
               params.armor_mono_proc_val.armor_angle_diff_min)
            {
                                        ...
                                        > (1 - params.armor_mono_proc_val.armor_length_retio_min))
                {
                             ...
                             )))
                    {
                        ...
                        float light_bars_x_diff = abs(light_bars.at(lb_idx).mid_p.x - light_bars.at(lb_idx_idx).mid_p.x);
                        float light_bars_avrg = (light_bars.at(lb_idx).length + light_bars.at(lb_idx_idx).length) / 2;
                        if((((light_bars_x_diff > params.armor_mono_proc_val.armor_length_mid_x_diff_small_l * light_bars_avrg) &&   \
                             (light_bars_x_diff < params.armor_mono_proc_val.armor_length_mid_x_diff_small_h * light_bars_avrg)) &&  \
                            params.robo_cmd.armor_type == RIN_SMALL_ARMOR)                                                           \
                           ||                                                                                                        \
                           (((light_bars_x_diff > params.armor_mono_proc_val.armor_length_mid_x_diff_big_l * light_bars_avrg) &&     \
                             (light_bars_x_diff < params.armor_mono_proc_val.armor_length_mid_x_diff_big_h * light_bars_avrg)) &&    \
                             params.robo_cmd.armor_type == RIN_BIG_ARMOR))
                        {
...
float ArmorMono::distance_slove(Point2f *apex_point, Params params)
{
    ...
    vector<Point2f> real_point;
        ...
        if(params.robo_cmd.armor_type == RIN_SMALL_ARMOR)
        {
        ...
        {
            solvePnP(big_armor_world_point, real_point, params.mono_cam_val.mono_cam_matrix_far, \
                     params.mono_cam_val.mono_cam_distcoeffs_far, rvec, tvec, false, CV_ITERATIVE);
        }
...
void ArmorMono::target_detect(void)
{
    ...
    for(size_t armor_idx = 0; armor_idx < armors.size(); armor_idx++)
    {
        ...
        if(curr_trust_lev > hightset_trust_lev)
        {
            ...
            target_info.X_offset = final_target.center.x - frame_info.frame_mid_x;
            target_info.Y_offset = final_target.center.y - frame_info.frame_mid_y;
            target_info.Z_offset = final_target.distance;
            hightset_trust_lev = curr_trust_lev;
            last_center = final_target.center;
        }
        target_info.target_flag = true;
    }
    if(target_info.target_flag == false)
    {
...
void ArmorMono::armor_mono_proc(Mat &in_img, Params params)
{
    ...
    string debug_lightbar_mid_x_diff_small_l = "lightbar mid_x diff_small_l: ";
    string debug_lightbar_mid_x_diff_small_h = "lightbar mid_x diff_small_h: ";
    string debug_lightbar_mid_x_diff_big_l = "lightbar mid_x diff_big_l: ";
    string debug_lightbar_mid_x_diff_big_h = "lightbar mid_x diff_big_h: ";
    ...
    if(params.robo_cmd.debug_flag == true)
    {
        ...
        putText(debug_cam_img, debug_lightbar_mid_y_diff, Point(0, 195), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 0), 1.5);
        debug_lightbar_mid_x_diff_small_l = debug_lightbar_mid_x_diff_small_l + \
                                            to_string(params.armor_mono_proc_val.armor_length_mid_x_diff_small_l);
        putText(debug_cam_img, debug_lightbar_mid_x_diff_small_l, Point(0, 210), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 0), 1.5);
        debug_lightbar_mid_x_diff_small_h = debug_lightbar_mid_x_diff_small_h + \
                                            to_string(params.armor_mono_proc_val.armor_length_mid_x_diff_small_h);
        putText(debug_cam_img, debug_lightbar_mid_x_diff_small_h, Point(0, 225), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 0), 1.5);
        debug_lightbar_mid_x_diff_big_l = debug_lightbar_mid_x_diff_big_l + \
                                            to_string(params.armor_mono_proc_val.armor_length_mid_x_diff_big_l);
        putText(debug_cam_img, debug_lightbar_mid_x_diff_big_l, Point(0, 240), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 0), 1.5);
        debug_lightbar_mid_x_diff_big_h = debug_lightbar_mid_x_diff_big_h + \
                                            to_string(params.armor_mono_proc_val.armor_length_mid_x_diff_big_h);
        putText(debug_cam_img, debug_lightbar_mid_x_diff_big_h, Point(0, 255), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 0), 1.5);
        debug_target_distance = debug_target_distance + to_string(final_target.distance);
        putText(debug_cam_img, debug_target_distance, Point(0, 270), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 0, 255), 1.5);

        if(params.robo_cmd.enemy_color == RIN_ENEMY_RED)
        {
            putText(debug_cam_img, "enemy red", Point(0, 285), \
                    DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 0, 285), 1.5);
        }
        else
        {
            putText(debug_cam_img, "enemy blue", Point(0, 285), \
        ...
        putText(debug_cam_img, debug_xyz, Point(0, 300), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 255), 1.5);
        debug_target_flag = debug_target_flag + to_string(target_info.target_flag);
        putText(debug_cam_img, debug_target_flag, Point(0, 315), \
                DEBUG_IAMGE_TEXT_FONT, 0.4, Scalar(0, 255, 255), 1.5);
